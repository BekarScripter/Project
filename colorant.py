...
class Colorant:
    ...
    def listen(self):
        while True:
            ...
            if win32api.GetAsyncKeyState(0x02) < 0 and self.toggled:
                ...
                self.process("click")
            elif win32api.GetAsyncKeyState(0x79) < 0 and self.toggled:
                self.process("flick")
    ...
    def process(self, action):
        ...
        if action == "move":
            ...
            y_diff = cY - self.grabber.yfov // 1.75
            self.arduinomouse.move(x_diff * self.movespeed, y_diff * self.movespeed)

        elif action == "click" and abs(center[0] - self.grabber.xfov // 2) <= 4 and abs(center[1] - self.grabber.yfov // 1.75) <= 10:
